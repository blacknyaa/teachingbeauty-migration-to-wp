...
## 設置時の注意（ローカル検証で確認済み）
- **サイドバー**：インストール直後は既定ウィジェットが入っているため、「外観 → ウィジェット」でサイドバーを空にすると、テーマのバナー＋店舗情報が表示される。
- **グローバルナビ**：「外観 → メニュー」で6項目（ホーム/当院について/治療内容・料金/キャンペーン・割引/無料相談/ご予約）を作成し、位置「グローバルナビ」に割当。
- **投稿日**：`teachingbeauty.wxr` は固定の過去日時（UTC）で生成済みのため、JSTサーバーでも「予約投稿」化しない（検証で対策済み）。

## 検証
本パッケージはローカルのWordPress（PHP 8.3 / SQLite）で取り込み・表示を確認済み。結果は `validation/VALIDATION.md` とスクリーンショットを参照。
